...
void PlotMatchedVertexCorrelationsMC() {
    ...
    const char *path1    = "/volatile/clas12/users/mfmce/data_jobs_rga_ppim_FLAG_MIN_MATCH_AND_FRACTION_DELTAP_9_13_23/skim_ppim_*.root";//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    const char *tree1    = "t";
    ...
    auto frame2 = d2//.Filter(cuts2)
      ...
      .Define("v_p", [](float vx_p, float vy_p, float vz_p) { return TMath::Sqrt(vx_p*vx_p+vy_p*vy_p+vz_p*vz_p); }, {"vx_p","vy_p","vz_p"})
      .Define("v_pim", [](float vx_pim, float vy_pim, float vz_pim) { return TMath::Sqrt(vx_pim*vx_pim+vy_pim*vy_pim+vz_pim*vz_pim); }, {"vx_pim","vy_pim","vz_pim"})
      .Define("vT_e_mc", [](float vx_e, float vy_e) { return TMath::Sqrt(vx_e*vx_e+vy_e*vy_e); }, {"vx_e_mc","vy_e_mc"})
      .Define("vT_p_mc", [](float vx_p, float vy_p) { return TMath::Sqrt(vx_p*vx_p+vy_p*vy_p); }, {"vx_p_mc","vy_p_mc"})
      .Define("vT_pim_mc", [](float vx_pim, float vy_pim) { return TMath::Sqrt(vx_pim*vx_pim+vy_pim*vy_pim); }, {"vx_pim_mc","vy_pim_mc"})
      .Define("v_e_mc", [](float vx_e, float vy_e, float vz_e) { return TMath::Sqrt(vx_e*vx_e+vy_e*vy_e+vz_e*vz_e); }, {"vx_e_mc","vy_e_mc","vz_e_mc"})
      .Define("v_p_mc", [](float vx_p, float vy_p, float vz_p) { return TMath::Sqrt(vx_p*vx_p+vy_p*vy_p+vz_p*vz_p); }, {"vx_p_mc","vy_p_mc","vz_p_mc"})
      .Define("v_pim_mc", [](float vx_pim, float vy_pim, float vz_pim) { return TMath::Sqrt(vx_pim*vx_pim+vy_pim*vy_pim+vz_pim*vz_pim); }, {"vx_pim_mc","vy_pim_mc","vz_pim_mc"})
      .Filter(cuts2); // NEEDED FOR CALCULATIONS LATER
    
    // Open output file
    TFile *f = TFile::Open("PlotMatchedVertexCorrelationsMC.root","RECREATE");
    f->cd();
    ...
    names.push_back("vT_p"); nbins.push_back(100); binlims.push_back({0.0,5.0}); labels.push_back("v_{T p} (cm)");
    names.push_back("vT_pim"); nbins.push_back(100); binlims.push_back({0.0,5.0}); labels.push_back("v_{T #pi^{-}} (cm)");
    ...
    //NOTE: Below are MC only
    names.push_back("vT_e_mc"); nbins.push_back(100); binlims.push_back({0.0,1.0}); labels.push_back("v_{T e^{-} MC} (cm)");
    names.push_back("vT_p_mc"); nbins.push_back(100); binlims.push_back({0.0,1.0}); labels.push_back("v_{T p MC} (cm)");
    names.push_back("vT_pim_mc"); nbins.push_back(100); binlims.push_back({0.0,1.0}); labels.push_back("v_{T #pi^{-} MC} (cm)");

    // names.push_back("v_e_mc"); nbins.push_back(100); binlims.push_back({0.0,.0}); labels.push_back("v_{e^{-} MC} (cm)");
    // names.push_back("v_p_mc"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{p MC} (cm)");
    // names.push_back("v_pim_mc"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{#pi^{-} MC} (cm)");

    names.push_back("vz_e_mc"); nbins.push_back(100); binlims.push_back({-5.0,0.0}); labels.push_back("v_{z e^{-} MC} (cm)");
    names.push_back("vz_p_mc"); nbins.push_back(100); binlims.push_back({-5.0,25.0}); labels.push_back("v_{z p MC} (cm)");
    names.push_back("vz_pim_mc"); nbins.push_back(100); binlims.push_back({-5.0,25.0}); labels.push_back("v_{z #pi^{-} MC} (cm)");

    //NOTE: Following only to be done if using mc matching cuts
    names.push_back("vz_parent_pim_mc"); nbins.push_back(100); binlims.push_back({-5.0,25.0}); labels.push_back("v_{z #Lambda MC} (cm)");

    // // Plot correlations data
    // const char *extraname1 = "2d_data";
    // for (int i=0; i<names.size(); i++) {
    //  if (i>0) break; //NOTE: Skip everything without mass_ppim as x variable
...
} // PlotMatchedVertexCorrelationsMC()
