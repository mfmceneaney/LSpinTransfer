...
void LMassFitGaussPoly4BGMC() {
    ...
    double sig_max_init = h->Integral()*TMath::Sqrt(2*TMath::Pi())*sigma_init/10;
    if ( myratio<1.5) { //lwdelVal<0.10) {//NOTE: MIGHT NEED TO TUNE THIS
      //sigma_init = 0.006;
      sig_max_init = h->Integral()*TMath::Sqrt(2*TMath::Pi())*sigma_init/10; //REDUCE SIGNAL COEFFICIENT
    ...
    if (delVal<0.15/*myratio2<1.1*/) { // THIS IS THE CASE WHEN YOU'RE CUTTING OUT LOTS OF HIGH MASS_PPIM BG AND BG SHAPE IS NO LONGER REALLY QUADRATIC...COULD FIND BETTER FUNCTION MAYBE...
      ...
      sigma_init = 0.009;
      sig_max_init = h->Integral()*TMath::Sqrt(2*TMath::Pi())*sigma_init/3;
    ...
    double par10 =  -beta;
    func->SetParameters(sigma_init,1.1157,sig_max_init,hmax,par6,par7,par8,par9,par10);
    func->SetParNames("sigma","Mu","C1","Pol2 max","Pol2 beta","Pol2 M0","Pol4 a8","Pol4 a9","Pol4 a10");
    ...
    double a8    = func->GetParameter(k++);
    double a9    = func->GetParameter(k++);
    double a10   = func->GetParameter(k++);

    // Crystal Ball fit errors
    k = 0; //IMPORTANT! RESET COUNTER
    ...
    double Ea8    = func->GetParError(k++);
    double Ea9    = func->GetParError(k++);
    double Ea10   = func->GetParError(k++);
