...
def get_arrs(out_file_list,x_means):
    ...
    # Get arrays
    x_mean    = np.array(x_means)#np.mean(glist[0],axis=1) #NOTE: Get mean across different bins (axis=1) but not across graphs (axis=0)
    y_mean    = np.mean(glist[1],axis=1)
...
def get_plots(
    ...
    yerr_syst = [0.00  for x in range(len(xbins)-1)] #NOTE: ADD IF STATMENT HERE #TODO #DEBUGGING !!!!!!!!!!!!!!!!!
