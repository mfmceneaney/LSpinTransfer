...
void PlotNegCorrelations() {
    ...
    int detector_identifier = 5; // 5; // 5-> Central, 6-> Forward
    std::string extraname = Form("data_rga_pos_detector_%d",detector_identifier); // "mc_rgh_neg";
    std::string extraname2d = Form("2d_%s",extraname.c_str());
    const char *path1    = Form("/volatile/clas12/users/mfmce/data_jobs_rga_posneg_3_13_24/%s*.root",skim_prefix.c_str());//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    const char *tree1    = "t";
    const char *cuts1    = Form("Q2>1 && W>2 && p_e>2.0 && vz_e>-25.0 && vz_e<20.0 && y<0.8 && detector%s==%d",particle_suffix.c_str(),detector_identifier);//"Q2>1 && W>2 && y<0.8 && xF_ppim>0.0 && z_ppim<1.0";
    ...
    // Open output file
    TFile *f = TFile::Open("PlotNegCorrelations.root","RECREATE");
    f->cd();
    ...
    names.push_back(Form("p%s",particle_suffix.c_str())); nbins.push_back(100); binlims.push_back({0.0,2.0}); labels.push_back(Form("p%s (GeV)",particle_title_suffix.c_str()));
    ...
    for (int i=0; i<names.size(); i++) {
      for (int j=0; j<names.size(); j++) {
        ...
        if (!((((i-j)%2)==0))) continue; //&& i<j && i>max_kinematics_idx && j>max_kinematics_idx) || (i<=max_kinematics_idx && j<=max_kinematics_idx))) continue; //NOTE: Creates block combos with just same particle and skip duplicates flipping axes
