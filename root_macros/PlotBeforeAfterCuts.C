...
* @date 7/Jul./23
* Description: e,p,pim MC Data kinematics comparisons before and after all kinematics cuts
*/
...
//   h_data->Scale(1/h_data->GetEntries());//NOTE: NORMALIZE FOR COMPARISON
  ...
  // Create histogram MC
  auto h2 = (TH1D) *d2.Histo1D({Form("h_with_cuts_%s",varName),varName,nbins,varMin,varMax},varName);
  TH1D *h_mc = (TH1D*)h2.Clone(Form("h_with_cuts_%s",varName));
//   h_mc->Scale(1/h_mc->GetEntries());//NOTE: NORMALIZE FOR COMPARISON
  ...
  TCanvas *c1 = new TCanvas(Form("c_data_with_cuts_%s",varName));
  c1->SetBottomMargin(0.125);
  c1->cd();
  THStack *h_stack = new THStack();
  h_stack->SetName(Form("hs_with_cuts_%s",varName));
  h_stack->Add(h_data);
  h_stack->Add(h_mc);
...
void PlotBeforeAfterCuts() {
    ...
    // const char *path2    = "/volatile/clas12/users/mfmce/mc_jobs_rga_ppim_7_7_23/skim_ppim_*.root";//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    // const char *tree2    = "t";
    const char *cuts2    =  "xF_ppim>0.0 && z_ppim<1.0 && mass_ppim<1.24 && Q2>1 && W>2 && y<0.8 && p_e>2.0 && vz_e>-25.0 && vz_e<20.0";
    // const char *drawopt  = "";//"PE1";
    ...
    // Open Data files with full cuts
    auto frame2 = d1//.Filter(cuts2) //NOTE: THIS IS D1 NOT D2 HERE!!!!!!!
      .Define("heli", "helicity") // TO ACCOUNT FOR WRONG HELICITY ASSIGNMENT IN HIPO BANKS, RGA FALL2018 DATA
      .Define("phi_e_2", [](float phi_e) { return (phi_e<0.0 ? 2*TMath::Pi()+phi_e : phi_e); }, {"phi_e"})
      .Define("phi_p_2", [](float phi_p) { return (phi_p<0.0 ? 2*TMath::Pi()+phi_p : phi_p); }, {"phi_p"})
      .Define("phi_pim_2", [](float phi_pim) { return (phi_pim<0.0 ? 2*TMath::Pi()+phi_pim : phi_pim); }, {"phi_pim"})
    ...
    names.push_back("z_ppim"); nbins.push_back(100); binlims.push_back({0.0,1.0}); labels.push_back("z_{p#pi^{-}}");
    ...
    // names.push_back("v_e"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{e^{-}} (cm)");
    ...
    // names.push_back("vz_e"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{z e^{-}} (cm)");
    // names.push_back("vz_p"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{z p} (cm)");
    // names.push_back("vz_pim"); nbins.push_back(100); binlims.push_back({0.0,10.0}); labels.push_back("v_{z #pi^{-}} (cm)");

    // // Plot correlations data
    // const char *extraname1 = "2d_data";              //NOTE: DON'T DO THIS ONE.  ALREADY DONE.
    ...
    // const char *extraname2 = "2d_data_with_cuts";
    // for (int i=0; i<names.size(); i++) {
    //   for (int j=0; j<names.size(); j++) {
    //     if (i==j) continue;//NOTE: SKIP IDENTITIES
    //     if (!(((i>8 && j>8) && j==i+3) || (i<=8 && j<=8))) continue; //NOTE: Creates block combos with just beta vs. p (y vs. x)
