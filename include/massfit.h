...
Double_t gauss_func(
   ...
   Double_t arg = 0;
   if (par[2]!=0) arg = (x[0] - par[1])/par[0];
   Double_t fitval = 0;
   if (par[2]!=0) fitval = par[2]*(1/(par[0]*TMath::Sqrt(2*TMath::Pi())))*TMath::Exp(-0.5*arg*arg);
   return fitval;

} // Double_t gauss_func()

Double_t gauss_and_quadratic_func(Double_t *x, Double_t *par) {
  
   Double_t arg = 0;
   if (par[5]!=0) arg = (x[0] - par[1])/par[0];
   Double_t fitval = 0;
   if (par[5]!=0) fitval = par[2]*(1/(par[0]*TMath::Sqrt(2*TMath::Pi())))*TMath::Exp(-0.5*arg*arg) + par[3] * (1 - par[4] * (x[0] - par[5]) * (x[0] - par[5]) );
   return fitval;

} // Double_t gauss_and_quadratic_func()

TArrayF* LambdaMassFit_Gaus(
                    const char * outdir,
                    TFile *outroot,
                    ROOT::RDF::RInterface<ROOT::Detail::RDF::RJittedFilter, void> frame,
                    const char *varName = "mass_ppim",
    ...
    // Set Fitting fn
    int nparameters = 6;
    TF1 *func = new TF1("fit",gauss_and_quadratic_func,varMin,varMax,nparameters);
    // func->SetParameters(0.5,2,0.006,1.1157,10000,h->GetBinContent(nbins),37,1.24);
    ...
    // Crystal Ball fit parameters
    double sigma = func->GetParameter(0);
    double mu    = func->GetParameter(1);
    double C1    = func->GetParameter(2);
    double a0    = func->GetParameter(3);
    double a1    = func->GetParameter(4);
    double a2    = func->GetParameter(5);

    // Crystal Ball fit errors
    double Esigma = func->GetParError(0);
    double Emu    = func->GetParError(1);
    double EC1    = func->GetParError(2);
    double Ea0    = func->GetParError(3);
    double Ea1    = func->GetParError(4);
    double Ea2    = func->GetParError(5);
    double chi2   = func->GetChisquare();
    double ndf    = func->GetNDF();
    ...
    // Save to file and return to above directory
    c1->SaveAs(Form("c1_gaus_%s.pdf",outdir));
    h->SaveAs(Form("h_gaus_%s.root",outdir));
    c1->Write(c1->GetName());
    h->Write(h->GetName());
    outroot->WriteObject(arr,"arr_gaus");
    outroot->cd("..");
...
TArrayF* LambdaMassFitMC_Gaus(
    ...
    // Save to file and return to above directory
    c1->SaveAs(Form("c1_gaus_%s.pdf",outdir_.c_str()));
    h->SaveAs(Form("h_gaus_%s.root",outdir_.c_str()));
    c1->Write(c1->GetName());
    h->Write(h->GetName());
    outroot->WriteObject(arr,"arr");
    outroot->cd("..");
