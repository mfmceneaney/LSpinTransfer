...
void plot(ROOT::RDF::RInterface<ROOT::Detail::RDF::RJittedFilter, void> d1_, std::vector<std::string> sector_types, int nsectors,
        const char *varName, int nbins, double varMin, double varMax, const char *varTitle, const char *drawopt, TFile *f) {
...
void PlotSectorCorrelationsMC() {
    ...
    // Open output file
    TFile *f = TFile::Open("PlotSectorCorrelationsMC.root","RECREATE");
    f->cd();
    ...
    // Get 1D Plots
    // plot(frame1,sector_types,nsectors,"Q2",100,1.0,10.0,"Q^{2} (GeV^{2})",drawopt,f);
    // plot(frame1,sector_types,nsectors,"W",100,2.0,4.5,"W (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"y",100,0.0,1.0,"y",drawopt,f);
    // plot(frame1,sector_types,nsectors,"x",100,0.0,1.0,"x",drawopt,f);

    // plot(frame1,sector_types,nsectors,"mass_ppim",100,1.08,1.24,"M_{p#pi^{-}}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"z_ppim",100,0.0,1.0,"z_{p#pi^{-}}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"xF_ppim",100,0.0,1.0,"x_{F p#pi^{-}}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"costheta1",100,-1.0,1.0,"cos(#theta) along P_{#Lambda}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"costheta2",100,-1.0,1.0,"cos(#theta) along P_{#gamma *}",drawopt,f);

    // plot(frame1,sector_types,nsectors,"pt_e",100,0.0,2.0,"p_{T e^{-}} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"pt_p",100,0.0,2.0,"p_{T p} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"pt_pim",100,0.0,2.0,"p_{T #pi^{-}} (GeV)",drawopt,f);

    plot(frame1,sector_types,nsectors,"p_e",100,0.0,10.0,"p_{e^{-}} (GeV)",drawopt,f);
    plot(frame1,sector_types,nsectors,"p_p",100,0.0,6.0,"p_{p} (GeV)",drawopt,f);
    plot(frame1,sector_types,nsectors,"p_pim",100,0.0,2.0,"p_{#pi^{-}} (GeV)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"pz_e",100,0.0,10.0,"p_{z e^{-}} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"pz_p",100,0.0,10.0,"p_{z p} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"pz_pim",100,0.0,10.0,"p_{z #pi^{-}} (GeV)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"theta_e",100,0.0,TMath::Pi(),"#theta_{e^{-}}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"theta_p",100,0.0,TMath::Pi(),"#theta_{p}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"theta_pim",100,0.0,TMath::Pi(),"#theta_{#pi^{-}}",drawopt,f);

    // plot(frame1,sector_types,nsectors,"phi_e_2",100,0.0,2*TMath::Pi(),"#phi_{e^{-}}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"phi_p_2",100,0.0,2*TMath::Pi(),"#phi_{p}",drawopt,f);
    // plot(frame1,sector_types,nsectors,"phi_pim_2",100,0.0,2*TMath::Pi(),"#phi_{#pi^{-}}",drawopt,f);

    plot(frame1,sector_types,nsectors,"theta_e_2",100,0.0,40.0,"#theta_{e^{-}} (deg.)",drawopt,f);
    ...
    // plot(frame1,sector_types,nsectors,"beta_pim",100,0.0,1.2,"#beta_{#pi^{-}} (GeV)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"chi2pid_e",100,-10.0,10.0,"#chi^{2}_{PID e^{-}} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"chi2pid_p",100,-10.0,10.0,"#chi^{2}_{PID p} (GeV)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"chi2pid_pim",100,-10.0,10.0,"#chi^{2}_{PID #pi^{-}} (GeV)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"vT_e",100,0.0,5.0,"v_{T e^{-}} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"vT_p",100,0.0,5.0,"v_{T p} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"vT_pim",100,0.0,5.0,"v_{T #pi^{-}} (cm)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"v_e",100,0.0,30.0,"v_{e^{-}} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"v_p",100,0.0,30.0,"v_{p} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"v_pim",100,0.0,30.0,"v_{#pi^{-}} (cm)",drawopt,f);

    // plot(frame1,sector_types,nsectors,"vz_e",100,-25.0,25.0,"v_{z e^{-}} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"vz_p",100,-25.0,25.0,"v_{z p} (cm)",drawopt,f);
    // plot(frame1,sector_types,nsectors,"vz_pim",100,-25.0,25.0,"v_{z #pi^{-}} (cm)",drawopt,f);
    ...
    for (int i=0; i<names.size(); i++) {
      for (int j=0; j<names.size(); j++) {
        ...
        if (i<3) { //NOTE: Do not plot by sector for phi variable
            plot2DNoSector(frame1,0,"sector_type",extraname2,names[i].c_str(),nbins[i],binlims[i][0],binlims[i][1],labels[i].c_str(),
                            names[j].c_str(),nbins[j],binlims[j][0],binlims[j][1],labels[j].c_str(),
                            drawopt,f);
        } else {
            plot2d(frame1,sector_types,nsectors,extraname2,names[i].c_str(),nbins[i],binlims[i][0],binlims[i][1],labels[i].c_str(),
                            names[j].c_str(),nbins[j],binlims[j][0],binlims[j][1],labels[j].c_str(),
                            drawopt,f);
        }
      }
