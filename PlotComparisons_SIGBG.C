...
void PlotComparisons_SIGBG() {
    ...
    const char *path1    = "/volatile/clas12/users/mfmce/mc_jobs_rga_ppim_FLAG_MIN_MATCH_AND_FRACTION_DELTAP_9_13_23/skim_ppim_*.root";//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    const char *tree1    = "t";
    const char *cuts1    = "mass_ppim<1.24 && Q2>1 && W>2 && p_e>2.0 && vz_e>-25.0 && vz_e<20.0 && xF_ppim>0.0 && z_ppim<1.0 && (pid_parent_p_mc==3122 && row_parent_p_mc==row_parent_pim_mc && abs(theta_p-theta_p_mc)<6.0*TMath::Pi()/180 && abs(theta_pim-theta_pim_mc)<6.0*TMath::Pi()/180)";//"Q2>1 && W>2 && y<0.8 && xF_ppim>0.0 && z_ppim<1.0";
    ...
    const char *tree2    = "t";
    const char *cuts2    = "mass_ppim<1.24 && Q2>1 && W>2 && p_e>2.0 && vz_e>-25.0 && vz_e<20.0 && xF_ppim>0.0 && z_ppim<1.0 && !(pid_parent_p_mc==3122 && row_parent_p_mc==row_parent_pim_mc && abs(theta_p-theta_p_mc)<6.0*TMath::Pi()/180 && abs(theta_pim-theta_pim_mc)<6.0*TMath::Pi()/180)";//"Q2>1 && W>2 && y<0.8 && xF_ppim>0.0 && z_ppim<1.0";
    // const char *drawopt  = "";//"PE1";
    ...
    // Open output file
    TFile *f = TFile::Open("PlotComparisons_SIGBG.root","RECREATE");
    f->cd();
    ...
    plot(frame1,frame2,"xF_ppim",100,0.0,1.0,"x_{F p#pi^{-}}",drawopt,f);
