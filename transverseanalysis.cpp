...
void analysis(const YAML::Node& node) {
    ...
    auto frame = (!inject_asym) ? d.Filter(cuts.c_str())
                    ...
                    .Define("XS", [&alpha,&bgasym,&sgasym,&dtheta_p_max,&dtheta_pim_max]
                        (float costheta, float ppid_p_mc, float pidx_p_mc, float pidx_pim_mc, float dtheta_p, float dtheta_pim) {
                            return (float)((ppid_p_mc==3122 && pidx_p_mc==pidx_pim_mc && dtheta_p<dtheta_p_max && dtheta_pim<dtheta_pim_max)
                        ...
                        {fitvar_mc.c_str(),"ppid_p_mc","pidx_p_mc","pidx_pim_mc","dtheta_p","dtheta_pim"})
