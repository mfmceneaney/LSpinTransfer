...
void analysis(const YAML::Node& node) {
    ...
    auto frame = (!inject_asym) ? d.Filter(cuts.c_str())
                    ...
                    .Define("XS", [&alpha,&bgasym,&sgasym,&dtheta_p_max,&dtheta_pim_max]
                        (float costheta, float pid_parent_p_mc, float row_parent_p_mc, float row_parent_pim_mc, float dtheta_p, float dtheta_pim) {
                            return (float)((pid_parent_p_mc==3122 && row_parent_p_mc==row_parent_pim_mc && dtheta_p<dtheta_p_max && dtheta_pim<dtheta_pim_max)
                            ? 0.5*(1.0 + alpha*sgasym*costheta) : 0.5*(1.0 + alpha*bgasym*costheta)); //NOTE: THIS ASSUMES THAT y and costheta are zero if no mc truth match found so then distribution is uniform.                  
                        },
                        {fitvar_mc.c_str(),"pid_parent_p_mc","row_parent_p_mc","row_parent_pim_mc","dtheta_p","dtheta_pim"})
