...
void analysis(const YAML::Node& node) {
    ...
    double sgasym2;
    if (node["sgasym2"]) {
        sgasym2 = node["sgasym2"].as<double>();
    }
    std::cout << "sgasym2: " << sgasym2 << std::endl;

    double bgasym2;
    if (node["bgasym2"]) {
        bgasym2 = node["bgasym2"].as<double>();
    }
    std::cout << "bgasym2: " << bgasym2 << std::endl;

    double sgasym3;
    if (node["sgasym3"]) {
        sgasym3 = node["sgasym3"].as<double>();
    }
    std::cout << "sgasym3: " << sgasym3 << std::endl;

    double bgasym3;
    if (node["bgasym3"]) {
        bgasym3 = node["bgasym3"].as<double>();
    }
    std::cout << "bgasym3: " << bgasym3 << std::endl;
    ...
    auto frame = (!inject_asym) ? d.Filter(cuts.c_str())
                    ...
                    .Define(fitvar_mc.c_str(),fitvarformula.c_str())
