...
void analysis(const YAML::Node& node) {
    ...
    std::string fitvar3 = "";
    if (node["fitvar3"]) {
        fitvar3 = node["fitvar3"].as<std::string>();
    }
    std::cout << "fitvar3: " << fitvar3 << std::endl;

    std::string fitvar3formula = "";
    if (node["fitvar3formula"]) {
        fitvar3formula = node["fitvar3formula"].as<std::string>();
    }
    std::cout << "fitvar3formula: " << fitvar3formula << std::endl;

    std::string fitvar3formulamc = "";
    if (node["fitvar3formulamc"]) {
        fitvar3formulamc = node["fitvar3formulamc"].as<std::string>();
    }
    std::cout << "fitvar3formulamc: " << fitvar3formulamc << std::endl;
    ...
    std::string fitvar3title = "y";
    if (node["fitvar3title"]) {
        fitvar3title = node["fitvar3title"].as<std::string>();
    }
    std::cout << "fitvar3title: " << fitvar3title << std::endl;
    ...
    int n_fitvar3_bins = 8;
    if (node["n_fitvar3_bins"]) {
        n_fitvar3_bins = node["n_fitvar3_bins"].as<int>();
    }
    std::cout << "n_fitvar3_bins: " << n_fitvar3_bins << std::endl;

    double fitvar3_min = 0.0;
    if (node["fitvar3_min"]) {
        fitvar3_min = node["fitvar3_min"].as<double>();
    }
    std::cout << "fitvar3_min: " << fitvar3_min << std::endl;

    double fitvar3_max = 1.0;
    if (node["fitvar3_max"]) {
        fitvar3_max = node["fitvar3_max"].as<double>();
    }
    std::cout << "fitvar3_max: " << fitvar3_max << std::endl;
    ...
    std::string fitvar3_mc = Form("%s_mc",fitvar3.c_str());//NOTE: CHANGE FITVAR->FITVAR_MC AFTER THIS FOR SANITY CHECKING MC ASYMMETRY INJECTION
    ...
    TF2 *fsgasyms = new TF2("fsgasyms",fsgasymsformula.c_str(),fitvar2_min,fitvar2_max,fitvar3_min,fitvar3_max); //NOTE: args: phi_h, y
    for (int idx=0; idx<sgasyms.size(); idx++) { fsgasyms->SetParameter(idx,sgasyms[idx]); }
    TF2 *fbgasyms = new TF2("fbgasyms",fbgasymsformula.c_str(),fitvar2_min,fitvar2_max,fitvar3_min,fitvar3_max); //NOTE: args: phi_h, y
    ...
    bool define_fitvars = (fitvar1formula.size()!=0 && fitvar2formula.size()!=0 && fitvar1formulamc.size()!=0 && fitvar2formulamc.size()!=0 && fitvar3formulamc.size()!=0 && fitvar3formulamc.size()!=0);
    if (define_fitvars) {
        d2 = (!inject_asym) ? d2
                                .Define(fitvar1.c_str(),fitvar1formula.c_str())
                                .Define(fitvar2.c_str(),fitvar2formula.c_str())
                                .Define(fitvar3.c_str(),fitvar3formula.c_str()) :
                                d2
                                .Define(fitvar1.c_str(),fitvar1formula.c_str())
                                .Define(fitvar2.c_str(),fitvar2formula.c_str())
                                .Define(fitvar3.c_str(),fitvar3formula.c_str())
                                ...
                                .Define(fitvar3_mc.c_str(),fitvar3formulamc.c_str());
    } // if (define_fitvars) {
    ...
    auto frame = (!inject_asym) ? d2.Filter(cuts.c_str())
                    ...
                    .Define("XS", [&fsgasyms,&fbgasyms,&alpha,&beam_polarization,&dtheta_p_max,&dtheta_pim_max]
                        ...
                        {fitvar1_mc.c_str(),fitvar2_mc.c_str(),fitvar3_mc.c_str(),"ppid_p_mc","pidx_p_mc","pidx_pim_mc","dtheta_p","dtheta_pim"})
    ...
    for (auto it = binvars.begin(); it != binvars.end(); ++it) { //TODO: How to check if too many binning variables...
        ...
        getKinBinnedGraphBSA3DGenericMCV2(
                    ...
                    fitvar3, // std::string  fitvarz = "y", // fitvariable branch name to use
                    fitvar3title, // std::string  fitvarztitle = "y", // fit variable axis title
                    n_fitvar3_bins, // int          n_fitvarz_bins = 8, // number of bins for fit variable if using LF method
                    fitvar3_min, // double       fitvarz_min = 0.0, // fit variable minimum
                    fitvar3_max, // double       fitvarz_max = 1.0, // fit variable maximum
