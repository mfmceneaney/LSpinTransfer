...
if __name__=="__main__":
    ...
    def apply_get_plots(out_file_list,get_outpath,get_plots,base_dir='',xlimss={},ylims=[0.0,1.0],titles={},xtitles={},ytitle='',verbose=True,aggregate_keys={},colors={},sgasyms=sgasyms): 
        ...
        for el in out_file_list:
            ...
            #DEBUGGING: BEGIN:
            print("DEBUGGING: bin_migration_mat = ",bin_migration_mat)#DEBUGGING
            bin_migration_mat_T = bin_migration_mat.transpose()
            print("DEBUGGING: bin_migration_mat_T = ",bin_migration_mat_T)#DEBUGGING
            bin_migration_mat_T_inv = np.linalg.inv(bin_migration_mat_T)#DEBUGGING
            print("DEBUGGING: bin_migration_mat_T_inv = ",bin_migration_mat_T_inv)#DEBUGGING
            unity = np.matmul(bin_migration_mat_T_inv,bin_migration_mat_T)
            print("DEBUGGING: unity = ",unity)
            y_mean_corrected = np.matmul(bin_migration_mat_T_inv,arrs['y_mean'])
            print("DEBUGGING: y_mean_corrected = ",y_mean_corrected)
            #DEBUGGING: END
            #----------#
