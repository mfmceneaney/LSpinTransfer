...
def get_plots(
    ...
    systematics = [0.05 for x in range(len(xbins)-1)] #NOTE: ADD IF STATMENT HERE #TODO
    ...
    plt.xlim(*xlims)
    plt.ylim(*ylims)
    # Add systematic uncertainty hist by percentage of g2[3]
    print("DEBUGGING: np.multiply(systematics,g1[3]) = ",np.multiply(systematics,g1[1]))#DEBUGGING
    s1 = plt.hist([1 for i in range(len(systematics))], weights=np.multiply(systematics,g1[1]), bins=xbins, color='gray', alpha=0.5)
    ...
    plt.ylim(*ylims)
    # Add systematic uncertainty hist by percentage of g2[3]
    s2 = plt.hist([1 for i in range(len(systematics))], weights=np.multiply(systematics,g2[1]), bins=xbins, color='gray', alpha=0.5)
...
if __name__=="__main__":
    ...
    method = 'HB'
    yaml_path = method+'_CT1/args.yaml' #NOTE: THIS ASSUMES BINNING SAME FOR BOTH CT1/CT2
    yaml_args = {}
