...
void getBinMigrationPlots(
    ...
    std::cout<<"DEBUGGING: MADE IT INTO getBinMigrationPlots() and converted bin limits arrays"<<std::endl;//DEBUGGING 1/3/25
...
void GetBinMigration2D__Inverse() {

    // Parameters for MC tree
    const char *path    = "/work/clas12/users/mfmce/mc_jobs_rga_ppim_2_23_24__BACKUP_LEGACY_DO_NOT_DELETE/skim_*.root";//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    const char *tree    = "t";
    ...
    std::cout<<"DEBUGGING: created dataframe"<<std::endl;//DEBUGGING: 1/3/25
    ...
    std::cout<<"DEBUGGING: created filtered dataframe"<<std::endl;//DEBUGGING: 1/3/25
    
    // Open output file
    TFile *f = TFile::Open("h_bin_migration_2D.root","RECREATE");
    f->cd();
    ...
    names.push_back("x"); titles.push_back("x"); names_mc.push_back("x_mc"); titles_mc.push_back("x_{MC}"); binlims.push_back(getBinLims(100,0.05,0.8));
    ...
    std::cout<<"DEBUGGING: created configuration"<<std::endl;//DEBUGGING: 1/3/25
