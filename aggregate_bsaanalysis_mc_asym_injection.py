        ...
        for xvar in var_lims:
            ...
            for key in aggregate_keys:
                ...
                for value in divisions[key]:
                    ...
                    job_dir = os.path.join(base_dir,"__".join(["_".join([key,"_".join([str(ele) for ele in data_list_i[key]]) if type(data_list_i[key])==list else str(data_list_i[key]) ]) for key in sorted(data_list_i)]))  #os.path.join(base_dir,"__".join(["_".join([key,str(data_list_i_val[key])]) for key in sorted(data_list_i_val)]))
...
if __name__=="__main__":
    ...
    sgasyms = {"sgasyms":[[a,b] for a in asyms for b in asyms]}
    bgasyms = {"bgasyms":[[a,b] for a in asyms for b in asyms]}
    ...
    base_dir    = "results_bsaanalysis_mc_asym_injection__3_25_24/" #"systematics/mc_asym_injection/"
