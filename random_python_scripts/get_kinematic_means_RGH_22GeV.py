...
def main(nthreads=16,path='*.root',tree='t',cuts='Q2>1',yaml_path='args.yaml',binvars_key='binvars',binvar_key='bins'):
    ...
    for binvar in binvars:
        ...
        filename = 'kinematics_22GeV_'+binvar+'.csv'
        delimiter = ','
...
if __name__=='__main__':
    ...
    # Set arguments here
    nthreads    = 2
    path        = '/volatile/clas12/users/mfmce/mc_jobs_rgh_22GeV_pipm_4_12_24/skim_pipim_*.root'
    tree        = 't'
    cuts        = 'Q2>1 && W>2 && y<0.8 && sqrt(px_e*px_e+py_e*py_e+pz_e*pz_e)>2.0 && vz_e>-25.0 && vz_e<20.0 && mx_pipim>1.5 && xF_pi>0.0 && xF_pim>0.0 && sector_pi!=4 && sector_pim!=4 && sector_e!=4 && TMath::Abs(TMath::ATan2(px_pi,pz_pi))<30.0*TMath::Pi()/180.0 && TMath::Abs(TMath::ATan2(py_pi,pz_pi))<25.0*TMath::Pi()/180.0 && TMath::Abs(TMath::ATan2(px_pim,pz_pim))<30.0*TMath::Pi()/180.0 && TMath::Abs(TMath::ATan2(py_pim,pz_pim))<25.0*TMath::Pi()/180.0 && TMath::Abs(TMath::ATan2(px_e,pz_e))<30.0*TMath::Pi()/180.0 && TMath::Abs(TMath::ATan2(py_e,pz_e))<25.0*TMath::Pi()/180.0' #NOTE: IMPORTANT! Only look at the signal region here.
    yaml_path   = 'results_pippimdihadronbsaanalysis_mc_asym_injection_rgh_22GeV_noSector4__4_19_24/args.yaml'
    binvars_key = 'binvars'
    binvar_key  = 'bins'
