...
def get_plots(
    ...
    if alternate_rg == 'rgc':
        ...
        rgh_data_counts /= (3.64/4.02) #(6.834/9.194) # /= (integrated_xs_rgh_mc/integrated_xs_rgc_mc) #NOTE: THE /= is important because the acceptance is N_events per unit XS #NOTE: OLD AS OF 10/21/24: /= (1.48/1.09)
...
if __name__=="__main__":
    ...
    base_dir    = "results_pippimdihadronbsaanalysis_mc_asym_injection_rgh_22GeV_noSector4__4_19_24/" #NOTE: DON'T FORGET ABOUT NOSECTOR4 SCENARIO
    alternate_rg = 'rgc'
    base_dir_csv_input = base_dir.replace('rgh',alternate_rg).replace('_noSector4','')#NOTE: ADDED 4/29/24
    output_dir_rga_data = base_dir.replace('_noSector4','').replace('mc_asym_injection','counts_mc').replace('rgh',alternate_rg).replace('mc','data').replace('_22GeV','')+"method_BSA/"
