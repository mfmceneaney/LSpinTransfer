...
void analysis(const YAML::Node& node) {
    ...
    auto frame = (!inject_asym) ? d2.Filter(cuts.c_str())
                    ...
                    .Define("ptpt",Form("phperp_%s * phperp_%s",suffix1.c_str(),suffix2.c_str()))
                    .Define(fitvar1.c_str(),[](float phi_1, float phi_2){ return (float) ((phi_1-phi_2)>=0 ? (phi_1-phi_2) : 2*TMath::Pi() + (phi_1-phi_2));},{phi_1_name.c_str(),phi_2_name.c_str()})
                    .Define(fitvar1_mc.c_str(),[](float phi_1, float phi_2){ return (float) ((phi_1-phi_2)>=0 ? (phi_1-phi_2) : 2*TMath::Pi() + (phi_1-phi_2));},{phi_1_name_mc.c_str(),phi_2_name_mc.c_str()})
                    ...
                    .Define("dtheta_k",[](float theta_k, float theta_k_mc){ return TMath::Abs(theta_k-theta_k_mc); },{"theta_k","theta_k_mc"})
      ...
      .Define("XS", [&fsgasyms,&fbgasyms,&beam_polarization,&dtheta_p_max,&dtheta_pim_max,&dtheta_k_max]
                        (float dphi, float ppid_p_mc, float pidx_p_mc, float pidx_pim_mc, float dtheta_p, float dtheta_pim, float dtheta_k) {
                            return (float)((ppid_p_mc==3122 && pidx_p_mc==pidx_pim_mc && dtheta_p<dtheta_p_max && dtheta_pim<dtheta_pim_max && dtheta_k<dtheta_k_max) ? 0.5*(1.0 + beam_polarization*fsgasyms->Eval(dphi)) : 0.5*(1.0 + beam_polarization*fbgasyms->Eval(dphi))); //NOTE: THIS ASSUMES THAT y and costheta are zero if no mc truth match found so then distribution is uniform.                  
                        },
