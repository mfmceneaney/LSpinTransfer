...
void analysis(const YAML::Node& node) {
    ...
    if (node["workspace_name"]) {
        workspace_name = node["workspace_name"].as<std::string>();
    }
    std::cout << "workspace_name: " << workspace_name << std::endl;

    std::string workspace_title = "workspace";
    if (node["workspace_title"]) {
        workspace_title = node["workspace_title"].as<std::string>();
    }
    std::cout << "workspace_title: " << workspace_title << std::endl;

    std::string dataset_name = "dataset";
    if (node["dataset_name"]) {
        dataset_name = node["dataset_name"].as<std::string>();
    }
    std::cout << "dataset_name: " << dataset_name << std::endl;

    std::string dataset_title = "dataset";
    if (node["dataset_title"]) {
        dataset_title = node["dataset_title"].as<std::string>();
    }
    std::cout << "dataset_title: " << dataset_title << std::endl;

    std::string sig_pdf_name1 = "cb";
    if (node["sig_pdf_name1"]) {
        sig_pdf_name1 = node["sig_pdf_name1"].as<std::string>();
    }
    std::cout << "sig_pdf_name1: " << sig_pdf_name1 << std::endl;

    std::string sig_pdf_name2 = "gauss";
    if (node["sig_pdf_name2"]) {
        sig_pdf_name2 = node["sig_pdf_name2"].as<std::string>();
    }
    std::cout << "sig_pdf_name2: " << sig_pdf_name2 << std::endl;

    std::string sgYield_name = "sgYield";
    if (node["sgYield_name"]) {
        sgYield_name = node["sgYield_name"].as<std::string>();
    }
    std::cout << "sgYield_name: " << sgYield_name << std::endl;

    std::string bgYield_name = "bgYield";
    if (node["bgYield_name"]) {
        bgYield_name = node["bgYield_name"].as<std::string>();
    }
    std::cout << "bgYield_name: " << bgYield_name << std::endl;

    double sg_region_min = 1.11;
    if (node["sg_region_min"]) {
        sg_region_min = node["sg_region_min"].as<double>();
    }
    std::cout << "sg_region_min: " << sg_region_min << std::endl;

    double sg_region_max = 1.13;
    if (node["sg_region_max"]) {
        sg_region_max = node["sg_region_max"].as<double>();
    }
    std::cout << "sg_region_max: " << sg_region_max << std::endl;

    int mass_nbins_conv = 1000;
    if (node["mass_nbins_conv"]) {
        mass_nbins_conv = node["mass_nbins_conv"].as<int>();
    }
    std::cout << "mass_nbins_conv: " << mass_nbins_conv << std::endl;
    ...
    for (auto it = binvars.begin(); it != binvars.end(); ++it) { //TODO: How to check if too many binning variables...
        ...
        getKinBinnedGraphGenericDiff(
                    ...
                    mass_nbins_conv,//int          mass_nbins_conv, // number of bins for convolution if needed for signal function
                    ...
                    workspace_name, //std::string workspace_name        = "w",
                    workspace_title, //std::string workspace_title       = "workspace",
                    dataset_name, //std::string dataset_name          = "dataset",//TODO add to overall arguments
                    dataset_title, //std::string dataset_title         = "dataset",//TODO add to overall arguments
                    sig_pdf_name1, //std::string sig_pdf_name1         = "cb",
                    sig_pdf_name2, //std::string sig_pdf_name2         = "gauss",
                    sgYield_name, //std::string sgYield_name          = "sgYield",//TODO add to overall arguments
                    bgYield_name, //std::string bgYield_name          = "bgYield",//TODO add to overall arguments
                    sg_region_min, //double sg_region_min              = 1.11,//TODO add to overall arguments
                    sg_region_max //double sg_region_max              = 1.1//TODO add to overall arguments
        );
