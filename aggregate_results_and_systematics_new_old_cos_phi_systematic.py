...
def get_plots(
    ...
    print("DEBUGGING: yaml_args = ",yaml_args)#DEBUGGING 5/24/24
    ...
    #TODO: DEBUGGING MESSAGE FOR BINS SEE IF SOMETHING GETS MESSED UP THERE AND MAKE SURE YOU ARE SETTING CORRECTLY...
    s1 = plt.hist(x_mean, weights=yerr_syst, bins=xbins, color='gray', alpha=0.5, label='(FD only)   Systematic Error') #NOTE: THAT HISTOGRAM X DATA SHOULD JUST USE BIN X MEANS SO THAT EACH BIN GETS ONE ENTRY AND THEN YOU CAN SCALE APPROPRIATELY WITH THE WEIGHTS ARGUMENT.
    s2 = plt.hist(x_mean_old, weights=-yerr_syst_old, bins=xbins, color='tab:orange', alpha=0.5, label='(CD and FD) Systematic Error')
    g2 = plt.errorbar(x_mean,y_mean,xerr=xerr_mean,yerr=yerr_mean,
                        ecolor=ecolor, elinewidth=elinewidth, capsize=capsize,
                        color=color, alpha=0.5, marker='o', linestyle=linestyle,
                        linewidth=linewidth, markersize=markersize,label='(FD only)   $D_{LL\'}^{\Lambda}$')
    g3 = plt.errorbar(x_mean_old,y_mean_old,xerr=xerr_mean_old,yerr=yerr_mean_old,
                        ecolor=ecolor, elinewidth=elinewidth, capsize=capsize,
                        color='tab:green'if color=='red' else 'tab:orange', alpha=0.5, marker='v', linestyle=linestyle,
                        linewidth=linewidth, markersize=markersize,label='(CD and FD) $D_{LL\'}^{\Lambda}$')
    plt.tick_params(direction='out',bottom=True,top=True,left=True,right=True,length=10,width=1)
    ax1.axhline(0, color='black',linestyle='-',linewidth=axlinewidth)
    plt.text(0.5, 0.5, 'CLAS12 Preliminary',
            size=50, rotation=25., color='gray', alpha=0.25,
    ...
    g_results_diffs = plt.errorbar(x_mean,pulls,xerr=None,yerr=None,
                        ...
                        linewidth=linewidth, markersize=markersize,label='(CD and FD) $\Delta D_{LL\'}^{\Lambda} / \sigma$')
    plt.tick_params(direction='out',bottom=True,top=True,left=True,right=True,length=10,width=1)
    ax1.axhline(0, color='black',linestyle='-',linewidth=axlinewidth)
    plt.text(0.5, 0.5, 'CLAS12 Preliminary',
            size=50, rotation=25., color='gray', alpha=0.25,
    ...
    # # Output the pull results with the new and old results there too
    # convert_graph_to_csv(
    #     outpath+'_new_old_comparison.csv',
    #     x_mean,
    #     y_mean,
    #     xerr=xerr_mean_old,
    #     yerr=yerr_mean_old,
...
if __name__=="__main__":
    ...
    def apply_get_plots(out_file_list,get_outpath,get_plots,base_dir='',xlimss={},ylims=[0.0,1.0],titles={},xtitles={},ytitle='',verbose=True,aggregate_keys={},colors={},input_yaml=input_yaml,systematics_function=None): 
        ...
        for el in out_file_list:
            ...
            inpath_csv = outpath.replace(old_dir,new_dir)+'.csv'
