...
void analysis(const YAML::Node& node) {
    ...
    std::string method = "BSA"; //NOTE: This can stay a std::string since it's a TString later...
    if (node["method"]) {
    ...
    if (node["costheta_name"]) {
        costheta_name = node["costheta_name"].as<std::string>();
    }
    ...
    std::string fitformula = "0.748*[1]*x"; // 1 + alpha * D_LL * (D(y)*cos(#theta))
    if (node["fitformula"]) {
