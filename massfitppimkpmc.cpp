...
void analysis(const YAML::Node& node) {
    ...
    double dtheta_k_max = 6.0*TMath::Pi()/180; //NOTE: IN RADIANS!
    if (node["dtheta_k_max"]) {
        dtheta_k_max = node["dtheta_k_max"].as<double>();
    }
    std::cout << "dtheta_k_max: " << dtheta_k_max << std::endl;
