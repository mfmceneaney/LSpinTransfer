...
def main(yaml_file, csv_file, scheme_name, hist_paths, hist_keys):
    # Load inputs
    ...
    for binvar, edges in scheme_bins.items():
        ...
        x_col = binvar.replace('_','')

        if x_col not in df.columns:
            raise KeyError(f"Missing column '{x_col}' in CSV")
        ...
        plot_hists(
            ...
            ylims=(0.0, 0.025 if binvar=="z_ppim" else 0.05),
            histtype="step",
            ...
            density=True,
            log=False,
            hist_labels=["CLAS12 Data"],
            binlims=edges,
            vlinestyle="dotted",
            vline_hist_idx=-1,
            legend_loc="lower left" if binvar=="z_ppim" else "lower right",
            hist_dim=1,
        )
        ...
        ax1.errorbar(
            ...
            color='tab:blue',
            linestyle=None,
            label="$\\pm\\sigma_{"+binvar_labels[binvar]+"}$",
        )
        ...
        print("df[ratio], = ",df["ratio"],)

        # Label
        xlims = (0.0,1.0) #(edges[0],edges[-1])
        ylims = (0.0,1.0)
        ax1.set_xlim(*xlims)
        ax1.set_ylim(*ylims)
        ax1.set_xlabel(f"${binvar_labels[binvar]}$",usetex=True)
        ax1.set_ylabel("$\\sigma_{"+binvar_labels[binvar]+"} / \\mathrm{Bin\\ width}$",usetex=True)
        ax1.set_title("Ratios of Bin Error to Width",usetex=True,pad=20)
        ax1.legend(loc="upper left" if binvar=="z_ppim" else "upper right")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="ROOT file keys for histograms to plot behind ratios",
    )
    ...
    # Run main program
    main(args.yaml, args.csv, args.scheme, args.hist_paths, args.hist_keys)
