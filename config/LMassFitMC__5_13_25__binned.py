...
def run(path='/RGA_MC_DIR/skim_*.root', tree='t', outname='LMassFit__5_23_25__binned_MC', extra_cut=None, outdir='.', pol=2):
    ...
    # MC truth-related angle cuts and matching used in other MC script
    dtheta_p_max = 6.0
    dtheta_pim_max = 6.0
    ...
    # Prepare MC truth selection expressions
    cuts_mc_base = f"({cuts}) && ({nomultiplicitycut})"
    mccuts = f"({cuts_mc_base}) && (ppid_p_mc==3122 && pidx_p_mc==pidx_pim_mc && ({angcuts}))"
    mccuts_true_proton = f"({cuts_mc_base}) && (ppid_p_mc==3122 && pidx_p_mc==pidx_pim_mc && (abs(theta_p-theta_p_mc)<{dtheta_p_max} && !(abs(theta_pim-theta_pim_mc)<{dtheta_pim_max})))"
    mccuts_true_pion = f"({cuts_mc_base}) && (ppid_pim_mc==3122 && pidx_p_mc==pidx_pim_mc && (!(abs(theta_p-theta_p_mc)<{dtheta_p_max}) && abs(theta_pim-theta_pim_mc)<{dtheta_pim_max}))"
    mccuts_true_bg = f"({cuts_mc_base}) && (!(ppid_p_mc==3122 && pidx_p_mc==pidx_pim_mc) || !({angcuts}))"
    ...
    base_lw = 2.0
    lw = base_lw * 1.5
    ax.step(bins[:-1], counts, where='post', label='Reco', color='k', linewidth=lw)
...
def main():
    ...
    parser.add_argument('--path', default=f"{os.environ.get('RGA_MC_DIR', '')}/skim_*.root", help='ROOT file glob path')
    parser.add_argument('--tree', default='t', help='Tree name')
    parser.add_argument('--outdir', default='binned_massfits_mc', help='Directory to save PDFs')
    ...
    BIN_SCHEMES = [
        { 'name': 'x', 'var': 'x', 'pol': [4,2,2,2,2], 'edges': [0.0000, 0.1515, 0.2033, 0.2564, 0.3339, 1.0] },
    ...
    for scheme in BIN_SCHEMES:
        ...
        for ib in range(nbins):
            ...
            print(f"Running MC for {outname} with cut: {extra_cut}")
