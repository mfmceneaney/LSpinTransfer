#!/usr/bin/env python3
"""PyROOT conversion of LMassFitMC_ppi__5_13_25__z_bin.C
...
def run(path='/RGA_MC_DIR/skim_*.root', tree='t', outname='LMassFitMC_ppi__5_13_25__z_bin'):
    ...
    pionangcuts = f"abs(theta_pi-theta_pi_mc)<{dtheta_pi_max:.8f}"
    ...
    cuts_all = (
        "mass_ppi<1.24 && Q2>1 && W>2 && y<0.8 && xF_ppi>0.0 && z_ppi<1.0 "
        "&& detector_p==6 && detector_pi==6 && sqrt(px_e*px_e+py_e*py_e+pz_e*pz_e)>2.0 "
        "&& vz_e>-10 && vz_e<2.5 && z_ppi>=0.6856 && z_ppi<0.7698"
    )
    cuts = f"{cuts_all} && ({nomultiplicitycut})"
    mccuts = f"({cuts}) && ({angcuts})"
    mccuts_true_proton = f"({cuts}) && ({true_proton_false_pion_cuts})"
    mccuts_true_pion = f"({cuts}) && ({false_proton_true_pion_cuts})"
    mccuts_true_bg = f"({cuts}) && !({angcuts})"
    ...
    func = TF1("fit", "[0]*(1 - [1]*(x-[2])*(x-[2]))", varMin, varMax)
    func.SetParameters(h.GetBinContent(nbins), 37, 1.24)
    func.SetParNames("Pol2 max", "Pol2 beta", "Pol2 M0")
    func.SetParLimits(0, h.GetBinContent(nbins) * 0.98, h.GetBinContent(nbins) * 10.0)
    ...
    bghist = func.GetHistogram().Clone('bghist')
    ...
    Ea0 = func.GetParError(0)
    Ea1 = func.GetParError(1)
    Ea2 = func.GetParError(2)
    chi2 = func.GetChisquare()
    ndf = func.GetNDF()

    # Format lines
    lines = [f"$\chi^2/NDF = {chi2/ndf:.2f}$",
             f"$a_{0} = {a0:.3f} \pm {Ea0:.3f}$",
             f"$a_{1} = {a1:.2f} \pm {Ea1:.2f}$",
             f"$a_{2} = {a2:.5f} \pm {Ea2:.5f}$"]
...
def main():
    ...
    parser.add_argument('--out', default='LMassFitMC_ppi__5_13_25__z_bin', help='Output name base')
