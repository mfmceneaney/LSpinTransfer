...
void GetTrueSignalFraction_LOWER_SIDEBAND() {

    // Parameters for MC tree
    const char *path    = "/RGA_MC_DIR/skim_*.root";//"~/clas12work/skim_Lambda_ROOT_12_9_20/*.root";
    const char *tree    = "t";
    const char *cuts    = "mass_ppim<1.24 && Q2>1 && W>2 && y<0.8 && xF_ppim>0.0 && p_e>2.0 && vz_e>-10 && vz_e<2.5 && detector_p==6 && detector_pim==6";//"Q2>1 && W>2 && y<0.8 && xF_ppim>0.0 && z_ppim<1.0";
    const char *cutsxF  = "mass_ppim<1.24 && Q2>1 && W>2 && y<0.8 && p_e>2.0 && vz_e>-10 && vz_e<2.5 && detector_p==6 && detector_pim==6"; //NOTE: REMOVE XF cut to get lower y contamination?
    const char *mccuts  = "Q2>1";
    const char *sigcut  = "mass_ppim>1.090 && mass_ppim<1.101";
    const char *drawopt = "APE";
    const char *name = "GetTrueSignalFraction_LOWER_SIDEBAND";
    ...
    // Open output file
    TFile *f = TFile::Open(Form("h_%s.root",name),"RECREATE");
    f->cd();
    ...
    int    nsteps         = 25;
    double bin_min        = 1.090;
    double bin_max        = 1.101;
...
} // GetTrueSignalFraction_LOWER_SIDEBAND()
