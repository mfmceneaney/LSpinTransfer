...
def get_plots(
    ...
    g3 = plt.errorbar(x_mean+0.05*(xlims[1]-xlims[0]),results_mcsmear,xerr=None,yerr=yerr_syst_mcsmear,
                ...
                linewidth=0, markersize=0,label='$\delta_{Syst} D_{LL\'}^{\Lambda}$ Smeared Momenta')
    g4 = plt.errorbar(x_mean+0.05*(xlims[1]-xlims[0]),results_mcsmear,xerr=xerr_mean,yerr=yerr_mean,
                        ...
                        linewidth=linewidth, markersize=markersize,label='$D_{LL\'}^{\Lambda}$ Smeared Momenta')
    plt.tick_params(direction='out',bottom=True,top=True,left=True,right=True,length=10,width=1)
    ax1.axhline(0, color='black',linestyle='-',linewidth=axlinewidth)
    # plt.text(0.5, 0.5, 'CLAS12 Preliminary',
    #         size=50, rotation=25., color='gray', alpha=0.25,
...
if __name__=="__main__":
    ...
    def apply_get_plots(out_file_list,get_outpath,get_plots,base_dir='',xlimss={},ylims=[0.0,1.0],titles={},xtitles={},ytitle='',verbose=True,aggregate_keys={},colors={},input_yaml=input_yaml,systematics_function=None): 
        ...
        for el in out_file_list:
            ...
            bin_migration_mat_mcsmear = load_TH2(path='GetBinMigration2D__Inverse__MCSmear__mom_only__5_14_25/sg/h_bin_migration_2D_final_bins.root',name='h2d_bin_migration_'+binvar)
            fmt_mcsmear = ["%d","%.3g","%.3g","%.3g","%.3g","%.3g"]
            save_matrix_to_csv(bin_migration_mat_mcsmear,base_dir='systematics_momc/bin_migration_mcsmear__mom_only/',binvar=binvar,fmt=fmt_mcsmear) #NOTE: SAVE BIN MIGRATION MATRIX TO CSV, MUST BE A SQUARE MATRIX!
