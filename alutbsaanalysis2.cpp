...
void analysis(const YAML::Node& node) {
    ...
    auto frame = (!inject_asym) ? d.Filter(cuts.c_str())
                    ...
                    .Define(depolarization_name.c_str(), [](float y) { return (y*TMath::Sqrt(1-y))/(1-y+0.5*y*y); }, {"y"}) :
                    ...
                    .Define(depolarization_name.c_str(), [](float y) { return (y*TMath::Sqrt(1-y))/(1-y+0.5*y*y); }, {"y"}) //NOTE: CHANGE y->y_mc FOR SANITY CHECKING MC ASYMMETRY INJECTION
                    .Define(depol_name_mc.c_str(), [](float y) { return (y*TMath::Sqrt(1-y))/(1-y+0.5*y*y); }, {"y_mc"}) // NEEDED FOR CALCULATIONS LATER
